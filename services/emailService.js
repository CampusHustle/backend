 ...
 * Replace the production branch with a real transporter once SMTP credentials are available.
 *
 * @param {{ to: string, subject: string, text: string }} options
 */
async function sendEmail({ to, subject, text }) {
  if (config.nodeEnv === 'test') {
    // Silent no-op in test environment — CI must not require an inbox or SMTP
    return;
  }
