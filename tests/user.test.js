...
test('User Model Schema - validates hourlyRate and search index fields', () => {
  ...
  assert.notEqual(schemaPaths.hourlyRate, undefined, 'hourlyRate should be defined on User schema');
  assert.equal(schemaPaths.hourlyRate.instance, 'Number');
  assert.notEqual(schemaPaths.skillsTeaching, undefined, 'skillsTeaching should be defined on User schema');
  assert.notEqual(schemaPaths.department, undefined, 'department should be defined on User schema');
  assert.ok(
    schemaPaths.rating !== undefined || schemaPaths['rating.knowledge'] !== undefined,
